...
class Route
{
    ...
    private static function _parseRule()
    {
        ...
        foreach ($routeconfig as $key => $config)
        {
     ...
     * @return string
     */
    private static function NoParam($url)
    {
        if (key_exists($url, self::$parseConfig))
        {
            $rou_text = self::$parseConfig[$url][0];
            $rou_url = str_replace(array('\\', '/^', '$/'), '', $rou_text);
     ...
     * @return string
     */
    private static function HasParam($url, $param)
    {
        $param_ext = '';
        $keys = $param;
        ...
        if (key_exists($rule, self::$parseConfig))
        {
            $rou_text = preg_split("/\((.*)\)/U", self::$parseConfig[$rule][0]);
            $rou_url = '';
            foreach ($rou_text as $k => $rt)
            {
                count($rou_text) - 1 == $k ? $rou_url.=$rt : $rou_url.=$rt . $param[self::$parseConfig[$rule][2][$k]];
            }
